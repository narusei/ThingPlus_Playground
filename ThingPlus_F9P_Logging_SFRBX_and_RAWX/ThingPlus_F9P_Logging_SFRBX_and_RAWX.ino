...
//File that all GNSS data is written to
OpenLog myLog;

#define sdWriteSize 465 // Write data to the SD card in blocks of 512 bytes
...
void loop()
{
  ...
  while (myGNSS.fileBufferAvailable() >= sdWriteSize) // Check to see if we have at least sdWriteSize waiting in the buffer
  {
    ...
    myLog.append("RXM_RAWX.ubx");
    for(int i=0; i<sdWriteSize/31; i++){
        myLog.write(myBuffer, 31);
        delay(15);
        myBuffer += 31;
    }
    myLog.syncFile();
    Serial.println("Written SD Card");

//    for(int i=0; i<sdWriteSize; i++){
//      myLog.write(myBuffer, 1);
//      Serial.print(*myBuffer, HEX);
//      myBuffer += 1;
//    }
//    myLog.syncFile();
